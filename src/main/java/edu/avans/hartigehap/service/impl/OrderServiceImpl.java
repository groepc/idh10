...
public class OrderServiceImpl implements OrderService {
	...
	public List<Order> findSubmittedOrdersForRestaurant(Restaurant restaurant) {
		...
		List<Order> submittedOrdersListAlternative = orderRepository.findByOrderStatusOrderStatusIdAndBillDiningTableRestaurant(
			OrderStatus.OrderStatusId.SUBMITTED, restaurant, new Sort(Sort.Direction.ASC, "submittedTime"));

		log.info("findSubmittedOrdersForRestaurant using query created using method name");
	...
	public List<Order> findPlannedOrdersForRestaurant(Restaurant restaurant) {
		// a query created using a repository method name
		List<Order> plannedOrdersList = orderRepository.findByOrderStatusOrderStatusIdAndBillDiningTableRestaurant(
			OrderStatus.OrderStatusId.PLANNED, restaurant, new Sort(Sort.Direction.ASC, "plannedTime"));
		return plannedOrdersList;
	}

	@Transactional(readOnly = true)
	public List<Order> findPreparedOrdersForRestaurant(Restaurant restaurant) {
		List<Order> preparedOrdersList = orderRepository.findByOrderStatusOrderStatusIdAndBillDiningTableRestaurant(
			OrderStatus.OrderStatusId.PREPARED, restaurant, new Sort(Sort.Direction.ASC, "preparedTime"));
