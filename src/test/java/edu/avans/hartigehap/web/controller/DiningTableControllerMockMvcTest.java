...
public class DiningTableControllerMockMvcTest {
	...
	public void dummy() throws Exception {
		...
		Object diningTableServiceMock = ReflectionTestUtils.getField(
				diningTableController, "diningTableService");
		assertTrue(diningTableServiceMock instanceof DiningTableService);
		String diningTableServiceMockClassName = diningTableServiceMock
				.getClass().getName();
		log.debug("diningTableServiceMockClassName: {}",
				diningTableServiceMockClassName);
		assertTrue(
				"diningTableServiceMockClassName contains 'Mock' since it is a mockito mock",
				diningTableServiceMockClassName.indexOf("Mock") >= 0);

		Object restaurantServiceMock = ReflectionTestUtils.getField(
				diningTableController, "restaurantService");
		assertTrue(restaurantServiceMock instanceof RestaurantService);
		String restaurantServiceMockClassName = restaurantServiceMock
				.getClass().getName();
		log.debug("restaurantServiceMockClassname: {}",
				restaurantServiceMockClassName);
		assertTrue(
				"restaurantServiceMockClassname contains 'Mock' since it is a mockito mock",
				restaurantServiceMockClassName.indexOf("Mock") >= 0);
	}

	/**
	 * verifies the following qualities of the DiningTableController.showTable():
	 * <ul>
	...
	public void showTable() throws Exception {
		...
		//execute
		Mockito.when(restaurantServiceMock.findAll()).thenReturn(
				restaurants);
		Mockito.when(diningTableServiceMock.fetchWarmedUp(1L)).thenReturn(diningTable);
		Mockito.when(restaurantServiceMock.fetchWarmedUp(RESTAURANT_ID))
				.thenReturn(restaurant);
		mockMvc.perform(get("/diningTables/1"))
				.andExpect(status().isOk())
				.andExpect(view().name("hartigehap/diningtable"))
				.andExpect(
						model().attribute(
								"restaurants",
								hasItems(restaurants.toArray(new Restaurant[] {}))));
	}

	
	/**
	 * verifies the following qualities of the DiningTableController.addMenuItem():
	 * <ul>
	 * <li>@RequestMapping annotation.</li>
	 * <li>HTTP return code</li>
	 * <li>view resolving: the correct view name is returned.</li>
	...
	public void addMenuItem() throws Exception {
		...
		.andExpect(view().name("redirect:/diningTables/1"))
		.andExpect(
				model().attribute(
						"diningTable",
						diningTable));
